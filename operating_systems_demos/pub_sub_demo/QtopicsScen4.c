...
int dequeue(int i) {
    ...
    int allowedDiff = 4;        // delete entries older than 4 seconds
    struct timeval now;
    int timediff;
...
void* subProxy(void * subEntry) {
    ...
    do {
        ...
        if (strcmp(buffer, "end") == 0) {
            ...
            // stop reading when there more than 10 seconds is passed since finding a new entry
                    ...
                    if (sub->topicList[i] != 0) {
                        ...
                        for (j = 0; j < MAXENTRIES; j++) {
                            ...
                            if (strcmp(readRecord[i][lastRead].message, curEntry.message) != 0) {
                                ...
                                // send topic msg back to sub
                                write(sub->quacker_to_sub[1], response, strlen(response)+1);

                                // wait for response for a certain time (3 seconds)
                                ...
                                if (subRespond == -1) {
                                    ...
                                    // for different implementation, would do something
                                }
...
void* del() {
    ...
    // terminate after 10 seconds of no deleting and after all threads have reached termination request(for safety)
