...
int getEntry(int topic, int lastEntry, struct topicentry * entry) {
    ...
    // -1 for pubID means no entry assigned
...
void* subProxy(void * subEntry) {
    ...
    do {
        ...
        if (strcmp(buffer, "end") == 0) {
            ...
            // read until over 10 seconds of not finding new entry
...
void* del() {
    ...
    // keep deleing while time since last delete is not over 10 secs or, as safety, till threads not dead
